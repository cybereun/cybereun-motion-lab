...
Cybereun Motion Lab은 오픈소스 Amicro 프로젝트를 기반으로 개인화하고 기능을 확장한 React 마이크로 인터랙션 컬렉션입니다. 운영 화면은 조금 더 밝은 남색의 **Motion Gallery Studio**로 구성되어 있습니다. 서로 다른 축소 미리보기에서 모션을 고르고, 집중형 중앙 무대에서 직접 체험하고, 필요할 때만 설정을 열어 조정한 뒤 구현 코드를 복사할 수 있습니다.
...
- **Motion Gallery Studio** — 서로 다른 미리보기 목록, 집중형 중앙 무대와 간결한 컬렉션 요약을 결합한 빠른 운영 UI입니다.
- **이중 탐색 방식** — 입장하면 보존된 원본 전체 보기가 먼저 표시되며, 필요할 때 집중형 Motion Gallery 개별 보기로 전환할 수 있습니다.
- **필요할 때만 여는 설정** — 미리보기 크기, 주변 광량과 포인트 색상을 화면을 가리지 않는 드로어에서 조절합니다.
...
│  ├─ AnimatedButton.tsx
│  └─ MotionWorkspace.tsx # Motion Gallery Studio 운영 화면
├─ data/              # 카탈로그 메타데이터
├─ utils/             # 코드 생성 및 복사 예제
